...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = ANALYSIS_PROMPT.replace('{workflow_info}', workflowInfo) + 
      '\n\nExtract the workflow information from this PDF document and analyze it for AI automation opportunities.';
    ...
    return NextResponse.json(
      { error: 'PDF analysis failed', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
